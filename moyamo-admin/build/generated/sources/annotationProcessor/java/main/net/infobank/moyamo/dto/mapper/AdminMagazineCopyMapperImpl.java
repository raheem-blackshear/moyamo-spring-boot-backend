...
public class AdminMagazineCopyMapperImpl implements AdminMagazineCopyMapper {

    @Override
    public MagazineWait of(PostingDto posting) {
        if ( posting == null ) {
            return null;
        }

        MagazineWait magazineWait = new MagazineWait();

        magazineWait.setYoutubeId( posting.getYoutubeId() );
        magazineWait.setCreatedAt( posting.getCreatedAt() );
        magazineWait.setModifiedAt( posting.getModifiedAt() );
        magazineWait.setId( posting.getId() );
        magazineWait.setText( posting.getText() );
        magazineWait.setAttachments( attachmentDtoListToPostingAttachmentList( posting.getAttachments() ) );
        magazineWait.setReadCount( posting.getReadCount() );
        magazineWait.setCommentCount( posting.getCommentCount() );
        magazineWait.setReportCount( posting.getReportCount() );
        magazineWait.setLikeCount( posting.getLikeCount() );
        magazineWait.setScrapCount( posting.getScrapCount() );
        magazineWait.setOwner( userDtoToUser( posting.getOwner() ) );
        magazineWait.setDelete( posting.isDelete() );
        magazineWait.setBlind( posting.isBlind() );
        magazineWait.setAdopt( posting.isAdopt() );
        magazineWait.setGoodses( goodsDtoListToGoodsList( posting.getGoodses() ) );
        magazineWait.setPosters( attachmentDtoListToPosterAttachmentList( posting.getPosters() ) );
        magazineWait.setTitle( posting.getTitle() );
